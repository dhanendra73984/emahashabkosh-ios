...
const Home = ({ route }: any) => {
  ...
  const handleSearchIconPress = () => {
    // 🔄 Clear previous search result immediately
    setSearchResult([]); // <-- This is the key fix
    setSoundAvailable(false); // Also reset sound availability if needed
...
const handleSoundIconPress = () => {
  ...
  const handleSuggestionSelect = (selectedSuggestion: string) => {
    ...
    // ✅ Automatically trigger the search when a suggestion is selected
    setTimeout(() => {
      handleSearchIconPress();
    }, 100); // slight delay to ensure state updates before API call
  };
