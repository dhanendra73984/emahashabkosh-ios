    ...
    axios.post('https://lilaonmobile.rb-aai.in/emaha/SailService.asmx/GetWordData', requestData, {
      ...
      .then(response => {
        ...
        if (apiData && Array.isArray(apiData) && apiData.length > 0) {
          ...
          // Check if audio file exists by making a separate request
          const audioUrl = `https://lilaonmobile.rb-aai.in/emaha/Audio/${encodeURIComponent(userSearch)}.mp3`; // Replace with actual audio URL
          axios.head(audioUrl)
            .then(() => {
              setSoundAvailable(true); // Set sound availability to true if the audio file exists
            })
            .catch(() => {
              setSoundAvailable(false); // Set sound availability to false if the audio file does not exist
            });
        } else {
          setSearchResult(['No data found for the given word.']);
          setSoundAvailable(false); // Set sound availability to false if no data is found
        }
        clearSearchBox();
        setSuggestions([]);
      })
      .catch(error => {
        ...
        clearSearchBox();
        setSuggestions([]);
        setSoundAvailable(false); // Set sound availability to false in case of an API error
      });
  };
...
const handleSoundIconPress = () => {
  ...
  const handleSearchInputChange = (text: string) => {
    ...
    console.log('Search Query:', text); // Move this line after setSearchQuery
  ...
  const handleSuggestionSelect = (selectedSuggestion: string) => {
    setUserSearch(selectedSuggestion); // Update userSearch when a suggestion is selected
